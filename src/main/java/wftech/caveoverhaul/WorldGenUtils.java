...
public class WorldGenUtils {
	...
	public static NoiseRouter overworld(HolderGetter<DensityFunction> hg1, HolderGetter<NoiseParameters> hg2, boolean p_255649_, boolean p_255617_) {
		...
		DensityFunction densityfunction16 = NoiseRouterDataAccessor.yLimitedInterpolatable(densityfunction15, DensityFunctions.noise(registry_np.get(Noises.ORE_VEININESS).get(), 1.5D, 1.5D), i, j, 0);
		float f = 4.0F;
		DensityFunction densityfunction17 = NoiseRouterDataAccessor.yLimitedInterpolatable(densityfunction15, DensityFunctions.noise(registry_np.get(Noises.ORE_VEIN_A).get(), 4.0D, 4.0D), i, j, 0).abs();
		DensityFunction densityfunction18 =NoiseRouterDataAccessor. yLimitedInterpolatable(densityfunction15, DensityFunctions.noise(registry_np.get(Noises.ORE_VEIN_B).get(), 4.0D, 4.0D), i, j, 0).abs();
