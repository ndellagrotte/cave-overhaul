...
public class GMatrix implements java.io.Serializable, Cloneable {
    ...
    // Compatible with 1.1
    @Serial
    private static final long serialVersionUID = 2777097312029690941L;
    ...
    public final void get(GMatrix m1) {
        ...
        nc = Math.min(nCol, m1.nCol);

        nr = Math.min(nRow, m1.nRow);
...
//            }
    ...
    static void compute_2X2(double f, double g, double h,
        ...
        if (ga == 0.0) {
            ...
            single_values[0] = d_sign(ssmax, tsign);
            d__1 = tsign * d_sign(c_b4, f) * d_sign(c_b4, h);
            single_values[1] = d_sign(ssmin, d__1);
        }
