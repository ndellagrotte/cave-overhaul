...
public class VirtualPackResources extends AbstractPackResources{
	...
	public VirtualPackResources(String vpName, boolean isBuiltIn) {
		...
		//WorldgenRevisited.LOGGER.error("[WorldgenRevisited] VirtualPackResources init! ");
		vpName = vpName;
	}
	...
	public void listResources(PackType p_10289_, String string1, String string2, ResourceOutput consumer) {
		//WorldgenRevisited.LOGGER.info("[WorldgenRevisited] VirtualPackResources: Finding and sending resources for " + string2);
		if(string2.equals("worldgen/configured_carver")) {
			//WorldgenRevisited.LOGGER.info("[WorldgenRevisited] VirtualPackResources: Sending " + STREAM_MAP_CARVER.keySet().size() + " entries for " + string2);
	...
	public void close() {
		for(ResourceLocation key: VirtualPackResources.STREAM_MAP.keySet()) {
			...
			try {
				...
				WorldgenRevisited.LOGGER.error("IOException: Attempting to close VirtualPackResources IOStream under key " + key + ", failed.");
				e.printStackTrace();
			}
		}
	}
